...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        with transaction.atomic():
            refund_orders = list(Order.objects.select_for_update(skip_locked=True).filter(
                status='refund_initiated',
                payment_status='paid',
                payment_method='razorpay'
            ).distinct())

            self.stdout.write(f"Found {len(refund_orders)} order(s) pending Razorpay refund...")
        ...
        # 2. Wallet Refunds
        with transaction.atomic():
            wallet_orders = list(Order.objects.select_for_update(skip_locked=True).filter(
                status='refund_initiated',
                payment_status='paid',
                payment_method='wallet'
            ).distinct())

            self.stdout.write(f"Found {len(wallet_orders)} order(s) pending Wallet refund...")
